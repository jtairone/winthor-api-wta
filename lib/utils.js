...
class Cliente {
  constructor (cliente){
    const mapeiaCampos = {
      ...
      emailNfe: { nome:'EMAILNFE', tipo:'string', tamanho: 3500 }, //STRING 3500 passado e-mail preenchera o campo PCCLIENT.EMAILNFE
    ...
    Object.entries(mapeiaCampos).forEach(([keyEn, config]) => {
      ...
      // Tratamento especial APENAS para email (primeira vez que encontra)
      if (keyEn === 'email' && (cliente['email'] !== undefined || cliente[config.nome] !== undefined)) {
        const emailsProcessados = this.processarEmails(cliente['email'] || cliente[config.nome]);
        this.email = this.converterValores(this.ajustarTamanhoString(emailsProcessados.email, mapeiaCampos.email.tamanho), 'string');
        this.emailNfe = this.converterValores(this.ajustarTamanhoString(emailsProcessados.emailNfe, mapeiaCampos.emailNfe.tamanho), 'string');
      } else if (keyEn === 'emailNfe') {
        // Pula emailNfe pois já foi processado com email
        // eslint-disable-next-line no-useless-return
        return;
      } else if (cliente[config.nome] !== undefined) {
  ...
  processarEmails (emailsString) {
    // Processa string de e-mails separados por ;
    // Retorna objeto com {email: primeiro_email, emailNfe: todos_emails}
    if (!emailsString) {
      return { email: '', emailNfe: '' };
    }
    ...
    return {
      email: emails[0], // Primeiro e-mail
      emailNfe: emails.join(';'),
    };
  }
