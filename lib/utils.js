...
class Cliente {
  constructor (cliente){
    ...
    Object.entries(mapeiaCampos).forEach(([keyEn, config]) => {
      ...
      // Tratamento especial para email e emailNfe
      if ((keyEn === 'email' || keyEn === 'emailNfe') && cliente['EMAIL'] !== undefined) {
        const emailsProcessados = this.processarEmails(cliente['EMAIL']);
